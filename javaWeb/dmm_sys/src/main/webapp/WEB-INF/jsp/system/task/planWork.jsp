          ...
          <div class="quanxian-tools">
            ...
            <div class="quanxian-block-search">
              <div class="quanxian-in-search"><span class="quanxian-fposition">
               计划名称： <input class="quanxian-search-t" type="text" placeholder="计划名称(该输入框不可编辑)"  ng-model="plan.name_" disabled>
  ...
  <div cs-window show="createPlanWorkDiv" autoCenter="true" offset="offset" title="'添加步骤'">
      ...
      <dd><input type="number" class="form-control" ng-model="planWork.order_" ></dd>
	 		...
	 		<div class="windowTable">
	 			...
	 			<table class="forterm-est">
   	<tr>
   	<td>
   	所属类型：<select ng-model="qWork.group_" >
   					<option value=""></option>
	 				<option ng-repeat="code in codes" value="{{code.code_}}">{{code.name_}}</option>
			</select>
	</td>
	<td> &nbsp;名称:<input type="text" ng-model="qWork.name_" ng-keyup="myKeyup($event)"></td>
   	<td><a href="" class="quanxian-search-q" ng-click="queryWork();" style="border: none;position: static;" title="搜索"></a></td>
   	</tr>
   	</table>
   	
<table  width="100%" border="0" cellpadding="0" cellspacing="0" class="table table-striped" init="vmw=[]">
			<thead>
	            <tr border="0">
	            <th></th>
	                <th>业务名称</th>
	       ...
	       cursor:pointer}
	        ...
	        <tr ng-repeat="item in vmW.items"  ng-click="checkWork(item);" ng-class="work.name_==item.name_?'vmwCheck':'vmwTR'">
						<td>{{$index+1}}</td>
						<td>{{item.name_}}</td>
