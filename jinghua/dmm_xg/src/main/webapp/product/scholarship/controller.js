...
.controller("controller", ['$scope','service', 'advancedService','$timeout','dialog', function($scope, service, advancedService,$timeout,dialog){
	...
	$scope.data = {
		...
		detailUrl : "pmsScholarship/getStuDetail",
		exportUrl : 'pmsScholarship/down',
		stu_detail : {
			page : {
				curpage :1,
				pagesize:10,
	...
	var sexDetail = function(param, obj, data){
		var detail = $scope.data.stu_detail,obj2={};
		detail.title  = param.name+"（"+param.seriesName+"）获得奖学金学生名单";
		detail.values = {};
		detail.headers= $scope.headers;
		detail.fields = $scope.fields;
		obj2.grade=obj.id,obj2.code=obj.list[param.seriesIndex].code;
		detail.values['sex']=obj2;
		distributionDetail();
	...
	var getTypeListFn = function(){
		service.getTypeList($scope.data.value_year, $scope.data.value_edu, getAdvancedParam(), function(list, option){
			addTitleOption(option,{text:'各奖学金对比',show:false});
			$scope.data.type = {
				list  : list,
				radar : option
			}
...
//				padding : [5,65]
	...
	var queryDeptDataListFn = function(){
		service.queryDeptDataList($scope.data.value_year, $scope.data.value_edu, getAdvancedParam(), function(bzdm, list, deptMc){
			var id = bzdm[0]["id"],
				option = service.getStackOptionByColumn(list, id, bzdm[0]["unit"], null, deptData_config,deptMc);
			//addTitleOption(option,{text:'奖学金'+deptMc+'分布（'+bzdm[0]["mc"]+'）',show:false});
	...
	$scope.changeDeptData = function(id,obj){
		...
		$scope.data.deptShow.show=false;
		addTitleOption(option,{text:'奖学金'+deptData.deptMc+'分布（'+obj.mc+'）',show:false});
	...
	var getBehaviorFn = function(){
		service.getBehavior($scope.data.value_year, $scope.data.value_edu, getAdvancedParam(), function(list, option){
			addTitleOption(option,{text:'获得奖学金学生行为',show:false});
			$scope.data.behavior = {
				list  : list,
				radar : option
			}
	...
	var getCoverageDeptFn = function(){
		service.getCoverageDept($scope.data.value_year, $scope.data.value_edu, getAdvancedParam(), function(bzdm, list, deptMc){
			...
			addTitleOption(option,{text:'奖学金'+deptMc+'累计覆盖率（'+mc+'）',show:false});
	...
	$scope.changeCoverageDept = function(id,obj){
		$scope.data.coverageDeptShow.show=true;
		...
		addTitleOption(option,{text:'奖学金'+coverageDept.deptMc+'累计覆盖率（'+coverageDept.mc+'）',show:false});
		addNoDataText2Option(option, '暂无'+coverageDept.deptMc+'覆盖率数据');
		coverageDept.option = option;
		// 判断其他提示信息
		getCoverageDeptInfoFn(option);
		$scope.data.coverageDeptShow.show=false;
	...
	var coverageGradeSex_config = {
		config : {
			title:{text:'奖学金男女生占比',show:false},
			legend : {
//				x : 'left',
//				padding : [5,65]
			}
	...
	var getCoverageGradeSexFn = function(){
		service.getCoverageGradeSex($scope.data.value_year, $scope.data.value_edu, getAdvancedParam(), function(list){
			var option = service.getStackOptionByColumn(list, "scale", "%", null, coverageGradeSex_config);
			addNoDataText2Option(option, '暂无年级性别分布数据');
			$scope.data.coverageGradeSex = {
				list   : list,
				option : option
			}
			getCoverageGradeSexInfoFn(option);
			Ice.apply(option, {config:{on:['CLICK', sexDetail]}});

		});
	}
	var getCoverageGradeSexInfoFn = function(option){
		var sortList = service.getSortDataFromOption(option, null), coverageGradeSex = $scope.data.coverageGradeSex,
			id = 0, sort_old = null, sort_asc = null, notZeroCount = 0, isSort = true, info = null;
		sortList.reverse(); // 从大四到大一才能比较是否是上升、下降；（'大一','大二','大三','大四' -> '大四','大三','大二','大一'） 20161201
		for(var i=0,len=sortList.length; i<len; i++){
			var id_one = sortList[i].id;
			if(id_one != 0) notZeroCount++;
			if(i != 0){
				if(id_one != 0){
					if(id_one >= id){
						sort_asc = true;
				...
				if(sort_old!=null && sort_old!=sort_asc){
					isSort = false; break;
				}else{
					sort_old = sort_asc;
				}
		...
		if(isSort && notZeroCount>=2){
			info = option.series[0].name+"：获奖比例逐年"+ (sort_asc ? "上升" : '下降');
		}
		coverageGradeSex.info = info;
	}
	...
	var getTopFn = function(){
		service.getTop($scope.data.value_year, $scope.data.value_edu, getAdvancedParam(), $scope.data.top.desc_column, $scope.data.top.index, function(data){
			$scope.data.top.list  = data.list;
			$scope.data.top.pageCount = data.pageCount;
		});
	}
	$scope.clickTop = function(desc_column){
		var top = $scope.data.top;
		if(top.desc_column == desc_column) return;
		top.desc_column = desc_column;
		top.index = 1;
		getTopFn();
	}
	$scope.clickTopPage = function(index_){
		var top    = $scope.data.top,
			index2 = top.index+index_;
		// 如果超出可计算范围，不再取数据
		if(index2 < 1 || index2 > top.pageCount)
			return;
		top.index = index2;
		getTopFn();
	}
...
//				padding : [5,65]
	...
	var getHistoryFn = function(){
		service.getHistory($scope.data.value_year, $scope.data.value_edu, getAdvancedParam(), function(bzdm, list){
			...
			addTitleOption(option,{text:'历年奖学金变化（'+bzdm[0]["mc"]+'）',show:false});
	...
	$scope.changeHistory = function(id, obj){
		$scope.data.historyShow.show=true;
		...
		addTitleOption(option,{text:'历年奖学金变化（'+obj.mc+'）',show:false});
		addNoDataText2Option(option, '暂无历年数据');
		history.option = option;
		history.id = id;
		$scope.data.historyShow.show=false;
