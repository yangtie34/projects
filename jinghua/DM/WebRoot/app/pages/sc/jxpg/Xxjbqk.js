 ...
 * 2015-01-07
 */
NS.define('Pages.sc.jxpg.Xxjbqk',{
    ...
    tplRequires : [{fieldname : 'maintpl',path : 'app/pages/sc/jxpg/tpl/xxjbqk.html'}],
	...
	refreshComp : function(xn){
		var me = this;
		var pageCode = "Pages.sc.jxpg.Xxjbqk";
	...
	bindEvents : function(){
		...
		$("#"+me.mainComp.id).dblclick(function(evt){
			...
			if($(tar)[0].tagName == "SPAN" && $(tar).attr("type") == "itemvalue"){
				$(tar).hide();
				$(tar).next().html("<input type='text' code='"+$(tar).attr("code")+
							"' value='"+$(tar).html()+"' name='"+$(tar).attr("name")+
							"' style='width:90%;height:26px;margin:5px;'>");
			}
		});
