...
public class EduUtils {
	...
	public static String getPasswordByIdno(String idno,String defaultValue){
		if("".equals(idno) || idno==null) { // 身份证号没有的值为默认值
			return defaultValue;
		}else if(idno.length()==18){ // 身份证12-17位
			return idno.substring(idno.length()-7, idno.length()-1);
		}else if(idno.length()==15){ // 身份证后六位
			return idno.substring(idno.length()-6, idno.length());
		}else{ // 身份证位数不正确的值为默认值
			return defaultValue;
		}
	}
	...
	public static void main(String[] args){
		System.out.println(getPasswordByIdno("012345678912345","5"));
