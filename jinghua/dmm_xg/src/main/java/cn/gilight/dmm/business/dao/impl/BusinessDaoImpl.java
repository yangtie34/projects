...
//					matchList.add("t.EDU_ID in("+values+")");
	...
	public String getStuScoreExamSql(String stuSql, String schoolYear, String termCode){
		...
		String scoreSql = "select t.stu_id, t.coure_code, t.credit,"
				...
				+ " (select t.stu_id, t.course_code coure_code, ch.credit, t.centesimal_score, t.hierarchical_score_code, ch.COURSE_ATTR_CODE, ch.COURSE_NATURE_CODE"
				+ " from T_STU_SCORE_HISTORY t, T_STU_COURSE_CHOOSE ch, ("+stuSql+") stu"
				+ " where t.cs=1 and t.stu_id=ch.stu_id and t.school_year=ch.school_year and t.term_code=ch.term_code and t.course_code=ch.scoure_code"
				+ " and t.stu_id=stu.no_ "+matchSql+" and ch.credit is not null"
				+ " union select t.stu_id, t.course_code coure_code, plan.credit, t.centesimal_score, t.hierarchical_score_code, plan.COURSE_ATTR_CODE, plan.COURSE_NATURE_CODE"
				+ " from T_STU_SCORE_HISTORY t, T_COURSE_ARRANGEMENT_PLAN plan, ("+stuSql+") stu"
				+ " where t.cs=1 and t.school_year = plan.school_year and t.term_code = plan.term_code and t.course_code = plan.course_code"
				+ " and t.stu_id = stu.no_ and plan.class_id = stu.class_id "+matchSql+" and plan.credit is not null )t"
	...
	public String getStuScoreSql(String stuSql, String schoolYear, String termCode){
		...
		String scoreSql = "select t.stu_id, t.coure_code, t.credit,"
				+ " case when t.hierarchical_score_code is not null then hier.centesimal_score else t.centesimal_score end score,"
				+ " t.COURSE_ATTR_CODE, t.COURSE_NATURE_CODE from "
				+ " (select t.stu_id, t.coure_code, ch.credit, t.centesimal_score, t.hierarchical_score_code, ch.COURSE_ATTR_CODE, ch.COURSE_NATURE_CODE"
				+ " from T_STU_SCORE t, T_STU_COURSE_CHOOSE ch, ("+stuSql+") stu"
				+ " where t.stu_id=ch.stu_id and t.school_year=ch.school_year and t.term_code=ch.term_code and t.coure_code=ch.scoure_code"
				+ " and t.stu_id=stu.no_ "+matchSql+" and ch.credit is not null"
				+ " union select t.stu_id, t.coure_code, plan.credit, t.centesimal_score, t.hierarchical_score_code, plan.COURSE_ATTR_CODE, plan.COURSE_NATURE_CODE"
				+ " from T_STU_SCORE t, T_COURSE_ARRANGEMENT_PLAN plan, ("+stuSql+") stu"
				+ " where t.school_year = plan.school_year and t.term_code = plan.term_code and t.coure_code = plan.course_code"
				+ " and t.stu_id = stu.no_ and plan.class_id = stu.class_id "+matchSql+" and plan.credit is not null )t"
				+ " left join t_code_score_hierarchy hier on t.hierarchical_score_code = hier.code_ where"
				+ " t.centesimal_score is not null or hier.centesimal_score is not null ";
		return scoreSql;
