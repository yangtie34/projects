...
*/
@Service
public class ProjectCompereServiceImpl implements ProjectCompereService{ 
	...
	public Integer queryTotalNums(String startYear,String endYear,String zzjgid,String shiroTag) {
		log.debug("查询作者人数！");
		String sql = "SELECT COUNT(DISTINCT NVL(T.TEA_NO,T.PEOPLE_NAME)) VALUE "
				+ "FROM T_RES_PROJECT_AUTH T"
				+ " INNER JOIN T_TEA TEA ON TEA.TEA_NO = T.TEA_NO "
				+ " INNER JOIN T_RES_PROJECT S ON T.PRO_ID = S.ID AND T.ORDER_ = 1"
				+ " AND S.SETUP_YEAR BETWEEN '"+startYear+"' AND '"+endYear+"'"
	...
	@Override
	public List<Map<String, Object>> queryAuthorNumsByGender(String startYear,String endYear,String zzjgid,String shiroTag) {
		log.debug("按照性别查询人数！");
		String sql = "SELECT COUNT(DISTINCT  NVL(T.TEA_NO,T.PEOPLE_NAME)) VALUE,NVL( SEX.NAME_,'未说明') NAME"
				+ " FROM T_RES_PROJECT_AUTH T"
				+ " INNER JOIN T_RES_PROJECT S ON T.PRO_ID = S.ID AND T.ORDER_ = 1"
				+ " AND S.SETUP_YEAR BETWEEN '"+startYear+"' AND '"+endYear+"'"
				+ "INNER JOIN T_CODE_DEPT D ON S.DEPT_ID = D.ID AND S.DEPT_ID IN "
					+ "	("+businessService.getDeptDataPermissIdsQuerySql(zzjgid, shiroTag)+")" 
				+ " INNER JOIN T_TEA TEA ON T.TEA_NO = TEA.TEA_NO"
	...
	*/
	@Override
	public List<Map<String, Object>> queryCompereNumsByProjectLevel(
		String startYear, String endYear, String zzjgid,String shiroTag){
		log.debug("项目的级别查询项目主持人*次分布");
		String sql = "SELECT COUNT(T.ID) VALUE, L.NAME_ NAME,l.order_"
				+ "  FROM T_RES_PROJECT_AUTH T"
				+ " INNER JOIN T_RES_PROJECT S ON T.PRO_ID = S.ID AND T.ORDER_ = 1"
				+ " AND S.SETUP_YEAR BETWEEN '"+startYear+"' AND '"+endYear+"'"
				+ "INNER JOIN T_CODE_DEPT D ON S.DEPT_ID = D.ID AND S.DEPT_ID IN "
					+ "	("+businessService.getDeptDataPermissIdsQuerySql(zzjgid, shiroTag)+")"
				+ "  INNER JOIN T_CODE L ON S.LEVEL_CODE = L.CODE_ AND L.CODE_TYPE = 'RES_PROJECT_LEVEL_CODE' where L.ISTRUE = 1"
				+ "  GROUP BY L.NAME_ ,l.order_ order by order_";
		return baseDao.queryListInLowerKey(sql);
	}
	
	@Override
	public List<Map<String, Object>>  queryAuthorNumsByEducation(String startYear,String endYear,String zzjgid,String shiroTag) {
		log.debug("查询学历分布！");
	...
	@Override
	public List<Map<String, Object>> queryAuthorNumsByAge(String startYear,String endYear,String zzjgid,String shiroTag) {
		log.debug("查询作者年龄分布！");
	...
	@Override
	public List<Map<String, Object>> queryAuthorNumsByZyjszwJb(String startYear,String endYear,String zzjgid,String shiroTag) {
		log.debug("作者职称类别！");
		String sql = " SELECT NVL(JB.NAME_,'未说明') NAME,jb.order_, COUNT(DISTINCT TEA.TEA_NO) VALUE"
				+ " FROM T_RES_PROJECT_AUTH T"
				+ " INNER JOIN T_RES_PROJECT S ON T.PRO_ID = S.ID AND T.ORDER_ = 1"
				+ " AND S.SETUP_YEAR BETWEEN '"+startYear+"' AND '"+endYear+"'"
				+ "INNER JOIN T_CODE_DEPT D ON S.DEPT_ID = D.ID AND S.DEPT_ID IN "
				...
				+ " GROUP BY  JB.NAME_ ,jb.order_ order by order_";
		return baseDao.queryListInLowerKey(sql);
	}
	
	@Override
	public List<Map<String, Object>> queryAuthorNumsByZyjszw(String startYear,String endYear,String zzjgid,String shiroTag) {
		log.debug("作者职称！");
	...
	@Override
	public List<Map<String, Object>> queryAuthorNumsByRylb(String startYear,String endYear,String zzjgid,String shiroTag) {
		log.debug("人员类别！");
	...
	@Override
	public List<Map<String, Object>> queryAuthorNumsByTeaSource(String startYear,String endYear,String zzjgid,String shiroTag) {
		log.debug("教职工来源！");
	...
	@Override
	public Map<String, Object> queryCompereListByPage(Page page,String startYear, String endYear, String zzjgid,String shiroTag
       ...
       " FROM T_RES_PROJECT_AUTH T"
		...
		+ "  LEFT JOIN T_CODE_TEA_SOURCE SC1 ON SC2.PID = SC1.ID"
		+ " LEFT JOIN T_CODE_EDUCATION E2 ON TEA.EDU_ID = E2.ID   AND E2.ISTRUE = 1"
		+ " LEFT JOIN T_CODE_EDUCATION E1 ON  E1.ID = E2.PID AND E1.LEVEL_ = 1  AND E1.ISTRUE = 11"
		+ " LEFT JOIN T_CODE_ZYJSZW Z2 ON TEA.ZYJSZW_ID = Z2.ID AND Z2.ISTRUE = 1"
		+ " LEFT JOIN T_CODE_ZYJSZW Z1 ON Z2.PID = Z1.ID AND Z1.ISTRUE = 1"
		+ " LEFT JOIN  t_code JB ON JB.CODE_ = Z2.ZYJSZW_JB_CODE AND JB.code_type = 'ZYJSZW_JB_CODE'"
