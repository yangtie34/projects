...
<!--左侧导航开始-->
			...
			<tbody>
				<tr ng-repeat="item in vm.items"  class="quanxian-bg-hover">
					...
					<td ng-if="item.istrue==0" ng-class="$index + 1==vm.items.length ? 'quanxian-sub-bottomsolid' : ''">
						...
						<a href="${ctx}/system/permssion/userid/{{item.id}}" class="quanxian-icon-a shuju-img-shouquan-icon"  title="角色授权">角色授权</a> 
					</td>
					<td>
  ...
  <div cs-window show="roleUpdateDiv" autoCenter="true" offset="offset" title="'编辑角色'">
      ...
      <dt>基本角色：</dt>
      <dd><input type="text" ng-model="userRootRole.DESCRIPTION" class="form-control" placeholder="角色标识" disabled></dd>
   	</dl>
   	<dt>  </dt>
      <dd ng-repeat="role in userRoles" style="margin-left: 100px;"><input type="checkbox" ng-model="role.ischeck"> {{role.DESCRIPTION}} </dd>
