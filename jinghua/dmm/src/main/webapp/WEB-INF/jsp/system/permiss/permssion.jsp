          ...
          <ul class="quanxian-select-group ">
            <li>
              <dl class="quanxian-select-group-dl clearfix">
                ...
                <dd>
                  <input type="text" class="quanxian-input-default" value="${flag eq 'role' ? role.description:user.username}（该输入框不可编辑）" disabled=""> 
                  <!-- <span class="quanxian-span-qiyong"> </span> -->
              ...
              <dl class="quanxian-select-group-dl clearfix">
                <dt>资源信息：</dt>
                <dd>
                 <input class="form-control" type="button" value="选择资源" ng-click="zyInfoWindow=!zyInfoWindow"/>
...
<table  width="100%" border="0" cellpadding="0" cellspacing="0" class="quanxian-sub-table">
	        ...
	        <tr ng-repeat="item in vm.items">
						<td>{{item.RESNAME}}</td>
						<td><a class="quanxian-alink" ng-click="item.DATAID==1?showJxjg(item.ID):showZzjg(item.ID);">{{item.DATANAME}}</a></td>
...
<div cs-window show="zyInfoWindow"  offset="offset" autoCenter="true" style="position: absolute">
				<div cg-combo-check-tree source="res_treeData" result="res_treeResult" 
...
<div cs-window show="zzjgInfoWindow"  offset="offset" autoCenter="true" style="position: absolute">
		<div cg-combo-check-tree source="zzjg_treeData" result="zzjg_treeResult" 
			code="'code'" treeType='zTree'>
			<!-- code 是条件对应的字段代码 -->
		</div>
			<div class="modal-footer">
...
<div cs-window show="jxjgInfoWindow"  offset="offset" autoCenter="true" style="position: absolute">
		<div cg-combo-check-tree source="jxjg_treeData" result="jxjg_treeResult" 
			code="'code'" treeType='zTree'>
			<!-- code 是条件对应的字段代码 -->
		</div>
				<div class="modal-footer">
...
<script type="text/javascript">
var res_treeData=JSON.parse('${resJson }');
var zzjg_treeData=JSON.parse('${zzjgJson }');
var jxjg_treeData=JSON.parse('${jxjgJson }');
var zzjg_treeData1={};
zzjg_treeData1=toAGLobj(zzjg_treeData,'0');
var jxjg_treeData1={};
jxjg_treeData1=toAGLobj(jxjg_treeData,'0');
/* 
var reg = /([^\:\{\}\[\]\,]+)\:([^\:\,\{\}\[\]]*)/g;
debugger;
var str='${dataSeverList }';
var reg_=new RegExp("=","g"); 
str=str.replace(reg_, ":");
reg_=new RegExp(" ","g"); 
str=str.replace(reg_, "");
str=str.replace(reg, "\"$1\":\"$2\"");

var addMc=function(obj){
	for(var i=0;i<obj.length;i++){
		obj[i].mc=obj[i].NAME_;
	}
	return obj;
};
var perm_dataTree= addMc(JSON.parse(str));
str='${operateList }';
str=str.replace(reg_, ":");
reg_=new RegExp("=","g"); 
str=str.replace(reg_, ":");
str=str.replace(reg, "\"$1\":\"$2\"");
var perm_dataTree2= addMc(JSON.parse(str)); */
</script>
</html>