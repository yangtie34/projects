...
/*
insert into  TL_CARD_PAY_ORIGIN_HOURS  
select to_date(substr(cp.time_, 0, 7),'yyyy-mm') year_month,            
        ...
        s.origin_name                                   
         ...
         inner join (
         select s.*,
         case when cad.id='650000' then '650000' else 'other' end origin_id, 
         case when cad.id='650000' then cad.name_ else '其他' end origin_name from
          t_stu s 
         ...
         where substr(cp.time_, 0, 7)='2016-11' 
...
*/
drop table TL_CARD_PAY_ORIGIN_HOURS;
create table TL_CARD_PAY_ORIGIN_HOURS
(
  YEAR_MONTH     DATE,
  HOUR_          VARCHAR2(100),
  PAY_MONEY      VARCHAR2(100),
  PAY_COUNT      VARCHAR2(100),
  CARD_DEAL_ID   VARCHAR2(100),
  CARD_DEAL_NAME VARCHAR2(100),
