          ...
          <div class="quanxian-tools">
            ...
            <div class="quanxian-block-search">
              <div class="quanxian-in-search"><span class="quanxian-fposition">业务名称：
                <input class="quanxian-search-t" type="text" placeholder="计划名称 模糊匹配"  ng-model="work.name_" disabled>
...
<!--content-->
  <div cs-window show="createWorkVerifyDiv" autoCenter="true" offset="offset" title="'添加验证'">
      ...
      <dd><input type="number" class="form-control" ng-model="workVerify.order_" ></dd>
   	...
   	<tr>
   	<td>
   	所属类型：<select ng-model="qVerify.group_" >
   					<option value=""></option>
	 				<option ng-repeat="code in codes" value="{{code.code_}}">{{code.name_}}</option>
			</select>
	</td>
	<td> &nbsp;名称:<input type="text" ng-model="qVerify.name_" ng-keyup="myKeyup($event)"></td>
   	<td><a href="" class="quanxian-search-q" ng-click="queryVerify();" style="border: none;position: static;" title="搜索"></a></td>
   	</tr>
   	</table>
<table  width="100%" border="0" cellpadding="0" cellspacing="0" class="table table-striped">
	       ...
	       cursor:pointer}
	        ...
	        <tr ng-repeat="item in vmW.items"  ng-click="checkVerify(item);" ng-class="verify.name_==item.name_?'vmwCheck':'vmwTR'">
					<td>{{$index+1}}</td>
					<td>{{item.name_}}</td>
