...
import com.jhnu.framework.entity.TsStsx;
...
public class ExportUtil {
	...
	public static HSSFWorkbook getHSSFWorkbookByCodes(List<String> strs,List<String> codes,String sheetName,List<Map<String,Object>> values){
		...
		HSSFCellStyle contentStyle = ExportUtil.createContentStyle(workbook);
		contentStyle.setWrapText(true);// 自动换行
		int sheetLength=50000;
		for(int k=0;k<values.size();k+=sheetLength){
			HSSFSheet sheet = workbook.createSheet(sheetName+k);
			//System.out.println("已分配内存中的剩余空间 = " + Runtime.getRuntime().freeMemory()/1024/1024);
			// 创建表标题
			HSSFRow header = sheet.createRow(0);
			...
			int lineNumber = 1;
			int thisSheetLength=(k+sheetLength)<values.size()?(k+sheetLength):values.size();
	    	for(int l=k;l<thisSheetLength;l++){
	    		Map<String,Object> map=values.get(l);
	    		HSSFRow contentRow = sheet.createRow(lineNumber++);
	    		contentRow.setHeight((short) 900);   // 设置行高
	    		int i=0;
