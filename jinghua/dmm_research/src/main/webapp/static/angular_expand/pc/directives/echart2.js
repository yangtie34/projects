 ...
 ************************************************/
system.directive('echart2', ['echartService',function (service) {
    return {
        restrict: 'AE',
        templateUrl : base + '/static/angular_expand/pc/directives/tpl/echart.html',
        scope: {
            config: "=",
            height: "@",
            onClick : "&"
        ...
        link : function(scope, element, attrs) {
        	...
        	element.find(".chartContainer").height(scope.height);
             scope.renderChart = function(){
            	// 基于准备好的dom，初始化echarts实例
            	 console.log(element.find(".chartContainer"));
            	 var myChart = echarts.init(element.find(".chartContainer")[0]);
                if(scope.config){
                	var options = {};
