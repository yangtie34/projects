...
public class HabitStuTypeDaoImpl implements HabitStuTypeDao{
	...
	public List<Map<String, Object>> getHabitZao(String startDate,
		...
		String sql="select sum(all_count) all_count,sum(all_money) all_money,code,name from (    "+
				...
				"else  3 end code      "+
				"from tl_card_stu_hour t where 1=1  "+tj+
	...
	public List<Map<String, Object>> getHabitZaoBySex(String startDate,
		...
		String sql="select sum(all_count) all_count,sum(all_money) all_money,code,name ,type_code,type_name from ( "+
				...
				"from tl_card_stu_hour t where 1=1"+tj+//" and t.card_deal_id in ("+CardTjUtil.ccx+")"+
	...
	public List<Map<String, Object>> getHabitZaoByArea(String startDate, String endDate,
		...
		String sql="select sum(all_count) all_count,sum(all_money) all_money,code,name ,type_code,type_name from ( "+
				...
				"when t.hour_ >'09' and t.hour_ <='14' then 2  "+
				"else  3 end code,     "+
				"nvl(t.province_id,'null') type_code,nvl(t.province_name,'未维护') type_name    "+
				"from tl_card_stu_hour t where 1=1"+tj+
				"group by t.province_id, t.province_name, t.hour_ ) group by type_code,type_name,code,name order by type_code,code ";
		return baseDao.getJdbcTemplate().queryForList(sql);
	}
	...
	public List<Map<String, Object>> getHabitHourByArea(String startDate, String endDate,
		...
		String tj=CardTjUtil.getDateTJ(startDate, endDate)+
				SqlUtil.getDeptTeachTj(deptTeach, ShiroTagEnum.CARD_HST.getCode(),"t");
		String sql="select nvl(t.province_id,'null') type_code,nvl(t.province_name,'未维护') type_name, "+
				"t.hour_, "+
				"sum(pay_money) all_money, "+
				"sum(pay_count) all_count "+
				"  from tl_card_stu_hour t  "+
				" where 1=1 "+tj+
				" group by t.province_id, t.province_name, t.hour_     "+
				" order by type_code, t.hour_";
		return baseDao.getJdbcTemplate().queryForList(sql);
	}
	...
	public List<Map<String, Object>> getHabitEatByArea(String startDate, String endDate,
		...
		String sql="select count(*) value,eat code,'日均'||eat||'餐' name,type_code,type_name from (   "+
				"select round(sum(a)/sum(value)) eat,stu_id,type_code,type_name from ( "+
				"select count(*) value,t.eat,count(*)*t.eat a,t.stu_id,nvl(t.province_id,'null') type_code,nvl(t.province_name,'未维护') type_name  "+
				"from tl_card_stu_eat t where 1=1 "+tj+
				"group by t.eat,t.stu_id,t.province_id,t.province_name )    "+
				"group by stu_id,type_code,type_name ) group by eat,type_code,type_name order by type_code,code ";
		return baseDao.getJdbcTemplate().queryForList(sql);
	}
	...
	public List<Map<String, Object>> getHabitZaoByMZ(String startDate,
		...
		String sql="select sum(all_count) all_count,sum(all_money) all_money,code,name ,type_code,type_name from ( "+
				...
				"when t.hour_ >'09' and t.hour_ <='14' then 2  "+
				"else  3 end code,     "+
				"nvl(t.nation_code,'null') type_code,nvl(t.nation_name,'未维护') type_name    "+
				"from tl_card_stu_hour t where 1=1"+tj+
				"group by t.nation_code, t.nation_name, t.hour_ ) group by type_code,type_name,code,name order by type_code,code ";
		return baseDao.getJdbcTemplate().queryForList(sql);
	}
	...
	public List<Map<String, Object>> getHabitHourByMZ(String startDate,
		...
		String tj=CardTjUtil.getDateTJ(startDate, endDate)+
				SqlUtil.getDeptTeachTj(deptTeach, ShiroTagEnum.CARD_HST.getCode(),"t");
		String sql="select nvl(t.nation_code,'null') type_code,nvl(t.nation_name,'未维护') type_name, "+
				"t.hour_, "+
				"sum(pay_money) all_money, "+
				"sum(pay_count) all_count "+
				"  from tl_card_stu_hour t  "+
				" where 1=1 "+tj+
				" group by t.nation_code, t.nation_name, t.hour_     "+
				" order by type_code, t.hour_";
		return baseDao.getJdbcTemplate().queryForList(sql);
	}
	...
	public List<Map<String, Object>> getHabitEatByMZ(String startDate,
		...
		String sql="select count(*) value,eat code,'日均'||eat||'餐' name,type_code,type_name from (   "+
				"select round(sum(a)/sum(value)) eat,stu_id,type_code,type_name from ( "+
				"select count(*) value,t.eat,count(*)*t.eat a,t.stu_id,nvl(t.nation_code,'null') type_code,nvl(t.nation_name,'未维护') type_name  "+
				"from tl_card_stu_eat t where 1=1 "+tj+
				"group by t.eat,t.stu_id,t.nation_code,t.nation_name )    "+
				"group by stu_id,type_code,type_name ) group by eat,type_code,type_name order by type_code,code ";
		return baseDao.getJdbcTemplate().queryForList(sql);
	}
}
