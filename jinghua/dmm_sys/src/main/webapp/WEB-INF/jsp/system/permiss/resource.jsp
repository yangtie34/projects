        ...
        <div class="quanxian-title-term">
          ...
          <div class="quanxian-btn-group">
           <!--  <button type="button" style="outline:none;" class="quanxian-btn-default"> 上级节点 </button> -->
            ...
            ng-click="addnewresource();"> 添加子菜单 </button>
          </div>
          </span>
           </div>
...
<table class="quanxian-sub-table" width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<tbody>
					<tr ng-repeat="item in vm.items" class="quanxian-bg-hover">
						<td>{{item.order_}}</td>
						<td>{{item.name_}}</td>
						<td>{{codeMap[item.resource_type_code]}}</td>
						<td>{{item.url_}}</td>
						<td>{{item.shiro_tag}}</td>
						<td>{{item.description}}</td>
						<td ng-show="item.resource_type_code!='02'"></td>
								<td ng-if="item.isShow==1&&item.resource_type_code=='02'" ng-class="$index + 1==vm.items.length ? 'quanxian-sub-bottomsolid' : ''">
						<a href="javascript:void(0)" ng-click="isShow(item)" class="quanxian-icon-a shuju-img-qiyong-icon" title="当前 显示 状态，点击隐藏">隐藏</a> 
					</td>
					<td ng-if="item.isShow==0&&item.resource_type_code=='02'" ng-class="$index + 1==vm.items.length ? 'quanxian-sub-bottomsolid' : ''">
						<a href="javascript:void(0)" ng-click="isShow(item)" class="quanxian-icon-a shuju-img-jinyong-icon" title="当前 隐藏状态，点击显示">显示</a> 
					</td>
...
<div cs-window show="addResource.flag" autoCenter="true" offset="offset" title="'资源'">
		<div>
			<input type="hidden" ng-model="resource.id" /> 
			   	<dl style="line-height: 47px;padding: 10px; " ng-show="selResDl.shiro_tag!=null&&selResDl.shiro_tag==selResDlTag">
      <dt style="float: left;
    text-align: right;
    width: 90px;
    margin-right: 10px;
    margin-top: 5px;">选择资源：</dt>
 		<dd>
 		<div cg-combo-box source="treeData" result="selResResult" on-select="boxSelect($data)" style="position: absolute;left: 130px;"></div>
 		<span style="float:right;padding: 10px; ">(*页面资源必选)</span>
 		</dd>
 		</dl>
      ...
      <input type="text" class="form-control"
				ng-model="resource.name_" placeholder="资源名称" /> 
				</dd>
   	</dl>
   	<dl class="quanxian-select-group-dl">
      <dt>资源描述：</dt>
      ...
      <label>{{shirotagPre}}</label>
      <input style="width: 145px;"class="form-control"
				type="text" ng-model="resourceshiro_tag" placeholder="权限标识" /> 
				</div>
				</dd>
   	</dl>
   	<dl class="quanxian-select-group-dl">
      ...
      <dd><input class="form-control"
   	   	...
   	   	<dl class="quanxian-select-group-dl" ng-show="false"><!-- ="thisNode.id=='0'" > -->
      <dt >系统类型：</dt>
      ...
      <dd> <select class="form-control"
				ng-model="resource.resource_type_code" ng-change="changeResType1();">
				<option value="">---请选择---</option>
				<option ng-show="thisNode.id=='0'||thisNode.id=='9999999999'" value="06">业务系统</option>
				<option ng-show="thisNode.id=='0'" value="07">组合系统</option>
				<c:forEach items="${resTypes }" var="resType">
					<option ng-show="thisNode.id>'0'&&${resType.code_ }!='07'" value="${resType.code_ }">${resType.name_ }</option>
				</c:forEach>
			</select></dd>
   	</dl>
	 	<dl class="quanxian-select-group-dl" ng-show="resource.resource_type_code=='02'||resource.resource_type_code=='06'||resource.resource_type_code=='07'" >
      <dt>资源路径：</dt>
      <dd><input class="form-control"
				ng-model="resource.url_" type="text" placeholder="资源路径" /> </dd>
   	</dl>
   		<dl class="quanxian-select-group-dl" ng-show="resource.resource_type_code=='02'">
      <dt>菜单显示：</dt>
      <dd>
        <div check-box-switch state="resourceisShow" result="resource.isShow" style="padding-top: 5px;">
  	</div>
      </dd>
    </dl>
   	 	<dl class="quanxian-select-group-dl">
      ...
      <dd>
        <div check-box-switch state="resourceistrue" result="resource.istrue" style="padding-top: 5px;">
		    ...
		    <button type="button" class="btn btn-primary" ng-show="!resourceUpdataflag" ng-click="insert_res()">添加</button>
			<button type="button" class="btn btn-primary" ng-show="resourceUpdataflag" ng-click="update_res()">确定</button>
	</div>	
		</div>
