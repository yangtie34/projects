/**
 * @class Pages.jw.jcgl.component.JcmkBaseModel
 * @extends Template.Page
 * @author haw_king(haw_king@foxmail.com)
 * 说明:
 *    headerData、showData、window.treeData(在tree继承页面)等都是模拟数据
 *    
 *  
 */
NS.define('Pages.qx.model.BaseModel', {
	/**
	 * 基础的模板定义名称
	 */
...
//    	this.initGrid(tableHeader,gridData);
        ...
        var tranData = this.tranData = NS.E2S(data['queryComponents']);//表头
      //查询form
		this.searchForm = this.createSearchForm({
				items:this.searchFormItem,
				data : tranData,
				enforcementComponent:this.grid
			},this);
//		this.searchForm.queryComponentByName('xnId').setValue(this.xnId);
//		this.searchForm.queryComponentByName('xqId').setValue(this.xqId);
        this.initTbar();
        this.initPageLayout();
...
//        var data = NS.util.DataConverter.entitysToStandards(headerData);
	...
	handlerColumnFn:function(btnValue,row,cell,data,name){
		...
		if(name=='rowupdate'){//审核
			...
			me.deleteIds();
		}else if(name=='rowsave'){
			me.editSave('save',function(backData){
		    	   //具体处理业务
		    	   this.saveInfoFn(backData);
		       });
		}
	},
	saveInfoFn:function(backData){
		if(backData.success){
 		   this.cancelForm();
 		   NS.Msg.info('保存成功！');
 		   this.reflash();
...
//*********************一般情况下重写上述部分即可*************************    
    ...
    initGrid: function (tableHeader,gridData) {
        ...
        if(this.edited==true){
        	mustCfg.plugins.push(new NS.grid.plugin.CellEditor());
        }
        this.grid = new NS.grid.Grid(mustCfg);
//        this.initGrid_(mustCfg);
       
    },
    createSecGrid:function(tableHeader,gridData){
    	//供form默认展现字段使用
    	this.defaultFormItems = NS.Array.pluck(tableHeader,'sx');//通过'sx'查找table里的各个字段名称
        //转换标准的表头数据
    	var columnData = this.tranData = NS.util.DataConverter.entitysToStandards(tableHeader);
        //如果要改变Grid的展现形式,请见updateGridBasicConfig
        var basic = this.secUpdateGridBasicConfig();
        //Grid展现必须的配置项
        var mustCfg = {
        		columnData: columnData,
        		serviceKey:{
        			key:'sec_queryGridData',
        			params:this.secPageParams()
        		},
                modelConfig: {
                    data : gridData
                },
        ...
        NS.apply(mustCfg,basic);
        var grid = new NS.grid.Grid(mustCfg);
        return grid;
    },
    secUpdateGridBasicConfig:function(){
    	var basic = {
                plugins: [new NS.grid.plugin.HeaderQuery()],
                autoScroll: true,
                border:false,
...
//    },
	 ...
	 * @param fn
	 */
    editSave:function(key,fn){
    	var modifyRows =this.grid.getModify();
    	if(modifyRows.length>0){
     	   var basic = {entityName:this.entityName,data:modifyRows[0]};
     	   this.callService({key:key,params:basic},function(data){
                     fn.call(fn,data[key]);
            },this);
        }	
  ...
  //创建查询form
	createSearchForm:function(cfg,scope){
    	...
    	form.bindItemsEvent({
    		...
    		reset:{event : 'click',fn : function(){
    			if(cfg.reset){
    				cfg.reset.call(scope||this,this);
    				return;
    			}
    			this.reset();
    		},scope : form}
	...
	findSx:function(data,name){
    	var comboData = null;
    	for(var i=0;i<this.tranData.length;i++){
        	var rec = this.tranData[i];
        	if(rec.name == name){
        		comboData =rec.codeData.data;
        		break ;
        	}
        }
    	return comboData;
