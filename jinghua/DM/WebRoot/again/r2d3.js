...
(function() {
  ...
  function cacher(f, scope, postprocessor) {
    function newf() {
      var arg = Array.prototype.slice.call(arguments, 0), args = arg.join("鈵€"), cache = newf.cache = newf.cache || {}, count = newf.count = newf.count || [];
      if (cache[has](args)) {
        repush(count, args);
  ...
  R.getRGB = cacher(function(colour) {
    ...
    if (rgb) {
      ...
      if (rgb[5]) {
        ...
        blue = toFloat(values[2]);
        values[2].slice(-1) == "%" && (blue *= 2.55);
        (values[0].slice(-3) == "deg" || values[0].slice(-1) == "掳") && (red /= 360);
...
window.Raphael.svg && function(R) {
  ...
  R.prototype.clear = function() {
    ...
    (this.desc = $("desc")).appendChild(R._g.doc.createTextNode("Created with Rapha毛l " + R.version));
...
d3 = function() {
  var 蟺 = Math.PI, 蔚 = 1e-6, d3 = {
    version: "3.0.8"
  }, d3_radians = 蟺 / 180, d3_degrees = 180 / 蟺, d3_document = document, d3_window = window;
  ...
  function d3_ease_elastic(a, p) {
    ...
    if (arguments.length) s = p / (2 * 蟺) * Math.asin(1 / a); else a = 1, s = p / 4;
    return function(t) {
      return 1 + a * Math.pow(2, 10 * -t) * Math.sin((t - s) * 2 * 蟺 / p);
    };
  }
  ...
  function d3_lab_hcl(l, a, b) {
    return d3_hcl(Math.atan2(b, a) / 蟺 * 180, Math.sqrt(a * a + b * b), l);
  }
  ...
  var d3_svg_arcOffset = -蟺 / 2, d3_svg_arcMax = 2 * 蟺 - 1e-6;
  ...
  d3.layout.pie = function() {
    var value = Number, sort = d3_layout_pieSortByValue, startAngle = 0, endAngle = 2 * 蟺;
    function pie(data) {
  ...
  function d3_geo_resample(project) {
    var 未2 = .5, maxDepth = 16;
    function resample(stream) {
      var 位0, x0, y0, a0, b0, c0;
      var resample = {
        point: point,
        lineStart: lineStart,
        lineEnd: lineEnd,
      ...
      function polygonLineStart() {
        var 位00, 蠁00, x00, y00, a00, b00, c00;
        lineStart();
        resample.point = function(位, 蠁) {
    ...
    function resampleLineTo(x0, y0, 位0, a0, b0, c0, x1, y1, 位1, a1, b1, c1, depth, stream) {
      ...
      if (d2 > 4 * 未2 && depth--) {
        ...
        if (dz * dz / d2 > 未2 || Math.abs((dx * dx2 + dy * dy2) / d2 - .5) > .3) {
  ...
  function d3_geo_albers(蠁0, 蠁1) {
    ...
    albers.invert = function(x, y) {
      var 蟻0_y = 蟻0 - y;
      return [ Math.atan2(x, 蟻0_y) / n, Math.asin((C - (x * x + 蟻0_y * 蟻0_y) * n * n) / (2 * n)) ];
    };
    return albers;
  }
  (d3.geo.albers = function() {
    var 蠁0 = 29.5 * d3_radians, 蠁1 = 45.5 * d3_radians, m = d3_geo_projectionMutator(d3_geo_albers), p = m(蠁0, 蠁1);
    p.parallels = function(_) {
      if (!arguments.length) return [ 蠁0 * d3_degrees, 蠁1 * d3_degrees ];
      return m(蠁0 = _[0] * d3_radians, 蠁1 = _[1] * d3_radians);
    };
  ...
  d3.geo.centroid = function(object) {
    ...
    var m;
    if (d3_geo_centroidW && Math.abs(m = Math.sqrt(d3_geo_centroidX * d3_geo_centroidX + d3_geo_centroidY * d3_geo_centroidY + d3_geo_centroidZ * d3_geo_centroidZ)) > 蔚) {
  ...
  function d3_geo_centroidPoint(位, 蠁) {
    ...
    d3_geo_centroidX += (cos蠁 * Math.cos(位) - d3_geo_centroidX) / d3_geo_centroidW;
    d3_geo_centroidY += (cos蠁 * Math.sin(位) - d3_geo_centroidY) / d3_geo_centroidW;
    d3_geo_centroidZ += (Math.sin(蠁) - d3_geo_centroidZ) / d3_geo_centroidW;
  }
  function d3_geo_centroidRingStart() {
    ...
    d3_geo_centroid.point = function(位, 蠁) {
      linePoint(位00 = 位, 蠁00 = 蠁);
    };
    d3_geo_centroid.lineEnd = function() {
      d3_geo_centroid.point(位00, 蠁00);
  ...
  function d3_geo_circleInterpolate(radians, precision) {
    ...
    return function(from, to, direction, listener) {
      if (from != null) {
        ...
        if (direction > 0 ? from < to : from > to) from += direction * 2 * 蟺;
      } else {
        from = radians + direction * 2 * 蟺;
        to = radians;
      }
      var point;
    ...
    return function(listener) {
      ...
      var clip = {
        ...
        polygonEnd: function() {
          ...
          } else if (visibleArea < -蔚 || invisible && invisibleArea < -蔚) {
            listener.lineStart();
      ...
      function point(位, 蠁) {
        if (pointVisible(位, 蠁)) listener.point(位, 蠁);
      }
  ...
  function d3_geo_clipSort(a, b) {
    return ((a = a.point)[0] < 0 ? a[1] - 蟺 / 2 - 蔚 : 蟺 / 2 - a[1]) - ((b = b.point)[0] < 0 ? b[1] - 蟺 / 2 - 蔚 : 蟺 / 2 - b[1]);
  }
  ...
  function d3_geo_clipAreaRing(ring, invisible) {
    ...
    var n, i = 0, area = 0, p = ring[0], 位 = p[0], 蠁 = p[1], cos蠁 = Math.cos(蠁), x0 = Math.atan2(invisible * Math.sin(位) * cos蠁, Math.sin(蠁)), y0 = 1 - invisible * Math.cos(位) * cos蠁, x1 = x0, x, y;
    while (++i < n) {
      p = ring[i];
      cos蠁 = Math.cos(蠁 = p[1]);
      x = Math.atan2(invisible * Math.sin(位 = p[0]) * cos蠁, Math.sin(蠁));
      y = 1 - invisible * Math.cos(位) * cos蠁;
      if (Math.abs(y0 - 2) < 蔚 && Math.abs(y - 2) < 蔚) continue;
      if (Math.abs(y) < 蔚 || Math.abs(y0) < 蔚) {} else if (Math.abs(Math.abs(x - x0) - 蟺) < 蔚) {
        if (y + y0 > 2) area += 4 * (x - x0);
      } else if (Math.abs(y0 - 2) < 蔚) area += 4 * (x - x1); else area += ((3 * 蟺 + x - x0) % (2 * 蟺) - 蟺) * (y0 + y);
      x1 = x0, x0 = x, y0 = y;
    }
    return area;
  }
  ...
  function d3_geo_clipAntimeridianLine(listener) {
    ...
    return {
      ...
      point: function(位1, 蠁1) {
        ...
        if (Math.abs(d位 - 蟺) < 蔚) {
          listener.point(位0, 蠁0 = (蠁0 + 蠁1) / 2 > 0 ? 蟺 / 2 : -蟺 / 2);
  ...
  function d3_geo_clipCircle(degrees) {
    ...
    function clipLine(listener) {
      ...
      return {
        ...
        point: function(位, 蠁) {
          var point1 = [ 位, 蠁 ], point2, v = visible(位, 蠁);
  ...
  (d3.geo.equirectangular = function() {
    return d3_geo_projection(d3_geo_equirectangular).scale(250 / 蟺);
  ...
  d3.geo.graticule = function() {
    ...
    return graticule.extent([ [ -180 + 蔚, -90 + 蔚 ], [ 180 - 蔚, 90 - 蔚 ] ]);
  };
  ...
  function d3_geo_mercator(位, 蠁) {
    return [ 位 / (2 * 蟺), Math.max(-.5, Math.min(+.5, Math.log(Math.tan(蟺 / 4 + 蠁 / 2)) / (2 * 蟺))) ];
  }
  d3_geo_mercator.invert = function(x, y) {
    return [ 2 * 蟺 * x, 2 * Math.atan(Math.exp(2 * 蟺 * y)) - 蟺 / 2 ];
  };
  ...
  function d3_geo_pathProjectStream(project) {
    var resample = d3_geo_resample(function(位, 蠁) {
      return project([ 位 * d3_degrees, 蠁 * d3_degrees ]);
    });
  ...
  function d3_geo_pathContext(context) {
    ...
    function point(x, y) {
      context.moveTo(x, y);
      context.arc(x, y, pointRadius, 0, 2 * 蟺);
    }
    function pointLineStart(x, y) {
      context.moveTo(x, y);
      stream.point = pointLine;
  ...
  function d3_geo_areaRingStart() {
    ...
    function nextPoint(位, 蠁) {
      ...
      var d位 = 位 - 位0, cos蠁 = Math.cos(蠁), sin蠁 = Math.sin(蠁), k = sin蠁0 * sin蠁, u0 = d3_geo_areaRingU, v0 = d3_geo_areaRingV, u = cos蠁0 * cos蠁 + k * Math.cos(d位), v = k * Math.sin(d位);
  ...
  function d3_geo_projectionMutator(projectAt) {
    ...
    }), k = 150, x = 480, y = 250, 位 = 0, 蠁 = 0, 未位 = 0, 未蠁 = 0, 未纬 = 0, 未x, 未y, clip = d3_geo_clipAntimeridian, clipAngle = null;
    function projection(point) {
  ...
  function d3_geo_projectionRadiansRotate(rotate, stream) {
    return {
      point: function(x, y) {
        ...
        stream.point(x > 蟺 ? x - 2 * 蟺 : x < -蟺 ? x + 2 * 蟺 : x, y[1]);
      },
      sphere: function() {
        stream.sphere();
      },
  ...
  function d3_geo_rotation(未位, 未蠁, 未纬) {
    return 未位 ? 未蠁 || 未纬 ? d3_geo_compose(d3_geo_rotation位(未位), d3_geo_rotation蠁纬(未蠁, 未纬)) : d3_geo_rotation位(未位) : 未蠁 || 未纬 ? d3_geo_rotation蠁纬(未蠁, 未纬) : d3_geo_equirectangular;
  }
  function d3_geo_forwardRotation位(未位) {
    return function(位, 蠁) {
      return 位 += 未位, [ 位 > 蟺 ? 位 - 2 * 蟺 : 位 < -蟺 ? 位 + 2 * 蟺 : 位, 蠁 ];
    };
  }
  ...
  function d3_geo_rotation蠁纬(未蠁, 未纬) {
    ...
    function rotation(位, 蠁) {
      ...
      return [ Math.atan2(y * cos未纬 - k * sin未纬, x * cos未蠁 - z * sin未蠁), Math.asin(Math.max(-1, Math.min(1, k * cos未纬 + y * sin未纬))) ];
    }
    rotation.invert = function(位, 蠁) {
      var cos蠁 = Math.cos(蠁), x = Math.cos(位) * cos蠁, y = Math.sin(位) * cos蠁, z = Math.sin(蠁), k = z * cos未纬 - y * sin未纬;
      return [ Math.atan2(y * cos未纬 + z * sin未纬, x * cos未蠁 + k * sin未蠁), Math.asin(Math.max(-1, Math.min(1, k * cos未蠁 - x * sin未蠁))) ];
    };
    return rotation;
  }
