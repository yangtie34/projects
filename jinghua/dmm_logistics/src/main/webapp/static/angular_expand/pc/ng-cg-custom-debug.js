		...
		link : function(scope, element, attrs) {
			...
			scope.$watch("isshow",function(){
				if(scope.isshow==undefined ){
					...
					attrs.$$element.slideUp(scope.time);
				}else{
					attrs.$$element.slideDown(scope.time);
				}
			},true);
		}
	};
...
jxpg.directive('cgComboSelect', ['$interval',function($interval) {
	return {
		restrict : 'AE',
		templateUrl : base + '/static/angular_expand/pc/directives/tpl/comboSelect.html',
		scope : {
			source:"=",
			result:"=",
			code : "="
...
jxpg.directive('cgComboCheckTree', ['$interval',function($interval) {
	return {
		restrict : 'AE',
		templateUrl : base + '/static/angular_expand/pc/directives/tpl/comboCheckTree.html',
		scope : {
			source:"=",
			result:"=",
			code : "=",
  ...
  return {
    ...
    link: function(scope, element, attrs, ctrls) {
      ...
      function getPages(currentPage, totalPages) {
        ...
        var p1=currentPage>5?currentPage-5:1;
        var p2=p1+9>endPage?endPage:p1+9;
        for (var number = p1; number <= p2; number++) {
...
jxpg.directive("jcScoreTjRange", [ function() {
	return {
		restrict : "AE",
		replace : true,
		templateUrl : base + '/static/angular_expand/pc/directives/tpl/jcScoreTjRange.html',
		scope : {
			cgMax : "=",//最大值（星星的个数）
			cgValueFrom : "=", //起始
			cgValueTo:"=",
...
jxpg.directive("jxpgTitle",["$window","dialog",function($window,dialog){
	return {
		restrict:'AE',
		replace:true,
		templateUrl:base + '/static/angular_expand/pc/directives/tpl/jxpgTitle.html',
		scope:{
			cgTitle:'@'
		},
...
jxpg.directive("cgScoreConvert", ["cgScoreConvertService", function(scoreConvertService) {
	return {
		restrict:'AE',
		replace:true,
		templateUrl:base + '/static/angular_expand/pc/directives/tpl/cgScoreConvert.html',
		scope:{
			cgValue:'='
		},
		link:function(scope,ele,attrs){
...
jxpg.directive("checkBoxSwitch",["$interval","$timeout",function($interval,$timeout){
	return {
		restrict:'AE',
		templateUrl:base + '/static/angular_expand/pc/directives/tpl/checkBoxSwitch.html',
		scope:{
			result:'=',
			state:'='
		},
...
jxpg.directive("quartzCron",["$interval","$timeout",function($interval,$timeout){
	return {
		restrict:'AE',
		templateUrl:base + '/static/angular_expand/pc/directives/tpl/quartzCron.html',
		scope:{
			result:'=',
			source:'='
		},
...
jxpg.directive('cgKcTable', function() {
	return {
		restrict : 'AE', 
		templateUrl : base
				+ '/static/angular_expand/pc/directives/tpl/cgKcTable.html',
		scope : {
			source : "=",
		},
		link : function(scope, element, attrs) {
...
jxpg.directive('cgComboNftj', ['$interval',function($interval) {
	return {
		restrict : 'AE',
		templateUrl : base + '/static/angular_expand/pc/directives/tpl/nftj.html',
		scope : {
			result:"=",
		},
		link : function(scope, element, attrs) {
			 ...
			 scope.$watch("year[2]",function(){
				 if(scope.year[2]!=null){
					 ...
					 var div2=element.find("button.dropdown-toggle:eq(1)");
...
jxpg.directive('cgComboXz', ['$interval',"$timeout","$compile",'mask','exportPage',function($interval,$timeout,$compile,mask,exportPage) {
	return {
		restrict : 'AE',
		templateUrl : base + '/static/angular_expand/pc/directives/tpl/comboXz.html',
		scope : {
			data:"=",
			type:"="
		},
		link : function(scope, ele, attrs) {
			...
			var xzpage = {
			 ...
			 scope.exportExcel=function(){
				 scope.page.exportExcel=true;
				var ex= scope.data.func(scope.page);
				exportPage.callService(ex).success(function(ret){
					
				})
				 scope.page.exportExcel=false;
				 mask.hideLoading();
			 }
			 scope.titlesClick=[];
...
jxpg.directive('cgComboNyrtj', ['$interval',"$timeout","$compile",'mask',function($interval,$timeout,$compile,mask) {
	return {
		restrict : 'AE',
		templateUrl : base + '/static/angular_expand/pc/directives/tpl/nyrtj.html',
		scope : {
			result:"=",
			yid:"@"
		},
		link : function(scope, ele, attrs) {
			 ...
			 //$compile(ele);
			 var getRes=function(){
					scope.result={startTime:scope.startTime,
