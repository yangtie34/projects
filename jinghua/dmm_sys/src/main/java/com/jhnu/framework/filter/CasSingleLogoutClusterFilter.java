...
public class CasSingleLogoutClusterFilter implements Filter {
	...
	/** Logger. */
	private static final Log LOG = LogFactory.getLog(CasSingleLogoutClusterFilter.class);
...
//	private static final String PEERS_PARAMETER = "peersUrls";
	...
	private static final String PEERS_USAGE = String.format(
			...
			PropertiesUtils.getDBPropertiesByName("sys.serverUrlNodes"));
			...
			if ("POST".equals(httpRequest.getMethod())) {
				...
				if (StringUtils.hasText(logoutRequest) && (rebroadcast == null)) {
					try {
						...
						CasSingleLogoutClusterFilter.LOG.debug("Got a single logout request ; protocol: ["+protocol+"] ; context: ["+context+"] ; path: ["+path+"].");
	...
	public void init(final FilterConfig config) throws ServletException {
		...
		this.clientHostName = PropertiesUtils.getDBPropertiesByName("sys.serverUrlNode");
		...
		if (!StringUtils.hasText(this.clientHostName)) {
			try {
				...
				final String errorMsg = String.format(
						...
						, PropertiesUtils.getDBPropertiesByName("sys.serverUrlNode"));
				CasSingleLogoutClusterFilter.LOG.error(errorMsg, e);
			}
		}

		// Get comma delimited list of peer dns names from a filter config
		final String peerList =PropertiesUtils.getDBPropertiesByName("sys.serverUrlNodes");
		...
		for (String peerDescription : peersDescription) {
			try {
				...
				CasSingleLogoutClusterFilter.LOG.error("Malformed peer URL [{}] among CasSingleLogoutClusterFilter peers !");
	...
	private final class MessageSender implements Callable<Boolean> {
		...
		protected boolean sendLogoutRequestToPeer(final Peer peer,
			...
			try {
				...
				CasSingleLogoutClusterFilter.LOG.error("Socket Timeout Detected while attempting to rebroadcast logout request {}");
			} catch (final Exception e) {
