(function(){
	EG.define("Sys.FP.GridChooser",{
		...
		constructor:function(cfg){
			...
			this.pop=new EG.ui.Dialog({
				...
				btns:[
					...
					new EG.ui.Button({text:"清空",click:function(){me.doClear();}		,cls:"eg_button_small",style:"margin:1px 5px;"})
				]
			});
		...
		setValue:function(value,d){
			if(d!=null) this.dMask.innerHTML=d[this.fieldName];
			this.value=value;
		},
		doSelect:function(){
		...
		setData:function(seg){
			...
			var me=this;
			Sys.call([
				{key:this.gridConfig["queryRPC"]["service"],params:{
					sqlWhere:sqlwhere,
					sqlGroup:[],
					sqlOrder:sqlorderby,
					startIndex:seg*30,
				...
				data=data[me.gridConfig["queryRPC"]["service"]];
				if(data!=null){
		...
		statics:{
			getTitle:function(value,data,cfg){return data?data[cfg["fieldName"]]:"";}
		}
	});
})();
