 ...
 */
directives.directive('csWindow',['$timeout',function($timeout){
	//创建遮罩
...
directives.directive('csSmartinput', ['$parse','$timeout', 'http', function ($parse,$timeout,http) {
	var temp =
	    ...
	    ' <div class="{{listclass}}" ng-show="listShow">' +
	    '  <ul>' +
	    '      <li  ng-repeat="item in list"  ng-click="selectOption(item,$index)" ng-transclude="child">' +
	    '      </li>' +
	    '  </ul>' +
	    '</div>' +
	    '      <input class="{{inputclass}}" ng-click="stopPropagation($event)"  ng-focus="focus()" ng-model="queryParam"  ng-change="doAjax(queryParam)" type="text" placeholder="{{placeholder}}" />' +
	    '</div>';
    ...
    return {
        ...
        link: function (scope, element, attrs) {
            var label = attrs.label,//input的标签
                ...
                inputClass = attrs.inputClass||"smart-input",//覆盖的input的class
                listClass = attrs.listClass||"smart-input-block",//覆盖的list的class
                displayField = attrs.displayField,//显示值字段
                setter = $parse(attrs.ngModel).assign;
            
                scope.customQuery = scope.customQuery||function(){return [];};//自定义查询
...
//            
            scope.$watch('value',function(value){
        		if(scope.listShow == false && (value == null||value == "")){
            ...
            scope.doAjax = createBuffer(function (queryParam) {
	                var param = [queryParam || null],
	                	extraParam = scope.customQuery(),
	                    req = {service : service,params : param};
	                
	                if(extraParam){
	                	param = param.concat(extraParam);
	                }
        ...
        parseParam=function(param, key){
            ...
            if(param instanceof String||param instanceof Number||param instanceof Boolean){
                ...
                $.each(param,function(i){
                    ...
                    paramStr+='&'+(this == window?(k+"="):parseParam(this, k));
                });
            }
