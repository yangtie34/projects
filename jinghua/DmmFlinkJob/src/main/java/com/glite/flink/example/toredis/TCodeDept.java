...
public class TCodeDept {
	...
	public static JDBCInputFormat createInputFormat() {
		...
		JDBCInputFormat jdbcInputFormat = JDBCInputFormat
				...
				.setPassword("dm")
				.setQuery(
						"select ID,CODE_,NAME_,PID,PATH_,LEVEL_,LEVEL_TYPE,ISTRUE,ORDER_,DEPT_CATEGORY_CODE from T_CODE_DEPT")
	...
	public static void main(String[] args) throws Throwable  {
		...
		DataSet<Tuple2<String, String>> dataSet=datasource.flatMap(new FlatMapFunction<Row, TCodeDept>() {
			@Override
			public void flatMap(Row value, Collector<TCodeDept> out) throws Exception {
				// TODO Auto-generated method stub
				
				out.collect(new TCodeDept(value.toString()));	
			}
		}).flatMap(new FlatMapFunction<TCodeDept, Tuple2<String,String>>() {
			@Override
			public void flatMap(TCodeDept value, Collector<Tuple2<String, String>> out) throws Exception {
				// TODO Auto-generated method stub
		...
		dataSet.output(new RedisOutPutFormat(jedisPoolConfig, new RedisHsetMapper("T_CODE_DEPT")));
				env.execute();
	...
	public TCodeDept(String values){
	   ...
	   this.setDept_category_code(vs[9]);
