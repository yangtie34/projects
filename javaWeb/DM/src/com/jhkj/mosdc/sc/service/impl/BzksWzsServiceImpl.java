...
import com.jhkj.mosdc.sc.service.BzksWzsService;

public class BzksWzsServiceImpl extends BzxServiceImpl implements
		BzksWzsService {
	...
	public String sendMail(String params) {
		...
		boolean flag =mailSender.sendMail4WZS(yxid,rq);;
		return "{success:"+flag+"}";
	}
	@Override
	public String queryGridContent(String params) {
		...
		String wgsql = "SELECT ZZJG.MC AS YX,ZZJG.ID,COUNT(*) as RS FROM TB_YKT_TEMP_Wzs WGMD "
				+ "LEFT JOIN TB_JXZZJG ZZJG ON TO_CHAR(ZZJG.ID) = WGMD.YX_ID "
				+ "WHERE ZZJG.MC IS NOT NULL AND WGMD.XFSJ LIKE '"
				+ rq
				+ "%' GROUP BY ZZJG.MC,ZZJG.ID";
		...
		String xsSql = "SELECT  JX.ID,JX.MC AS YX,XSZT.MC AS XSZT,COUNT(*) AS RS FROM TB_XJDA_XJXX XJXX "
				...
				+ "WHERE XJZT_ID = 1000000000000101 AND XSZT_ID = 1 " 
				+"and substr(xjxx.yjbysj,0,4) " +
				((month < 7)?">=":">" ) +
		...
		for (Map temp : yxxss) {
			...
			// 获取消息通知状态
			String xxztSql = "SELECT * FROM TB_YKT_TEMP_Wzs_MAIL MAIL WHERE JXZZJGID="
					+ yxid + " AND DYRQ='" + rq + "'";
		...
		if(json.containsKey("dcqb")){
			...
			for(Map temp : yxList){
				...
				String wgMdSql ="SELECT T.* FROM (SELECT WGMD.XH,ZZJG.MC YX,ZZJG.ID YX_ID,XJ.LXDH,ZY.MC AS ZY,ZSXX.ZSDD,XJ.XM,WGMD.XFSJ SCXFSJ,WGMD.SKSJ SCMJSJ,WGMD.TJSJ "+ 
						...
						" WHERE ZZJG.MC IS NOT NULL AND WGMD.XFSJ LIKE '"+rq+"%' AND ZZJG.ID = "+yx_id+" ORDER BY YX,ZY,XH DESC)T";
	...
	public String queryWzsMd(String params) {
		...
		String wgMdSql ="SELECT T.* FROM (SELECT WGMD.XH,ZZJG.MC YX,ZZJG.ID YX_ID,XJ.LXDH,ZY.MC AS ZY,ZSXX.ZSDD,XJ.XM,WGMD.XFSJ SCXFSJ,WGMD.SKSJ SCMJSJ,WGMD.TJSJ "+ 
									...
									" LEFT JOIN (SELECT ZY.XS_ID,LY2.MC||'/'||LY.MC||'/'||FJ.MC||'/'||CW.MC||'号床' ZSDD FROM TB_DORM_ZY ZY ,TB_DORM_CW CW ,TB_DORM_CCJG FJ,TB_DORM_CCJG LY,TB_DORM_CCJG LY2 "+
									"WHERE ZY.CW_ID = CW.ID AND CW.FJ_ID = FJ.ID AND FJ.FJD_ID = LY.ID AND LY.FJD_ID = LY2.ID) ZSXX  ON ZSXX.XS_ID = XJ.XH "+
									" WHERE ZZJG.MC IS NOT NULL AND WGMD.XFSJ LIKE '"+rq+"%' AND ZZJG.ID = "+yxid+tcSql+" ORDER BY YX,ZY,XH DESC)T";
	...
	public String queryYxglyJszt(String params) {
		...
		String sql ="SELECT T.* FROM (SELECT YX.ID AS YXID,JZG.DZXX,JZG.LXDH,YX.MC AS YX,JZG.ZGH,JZG.XM,"+
			...
			" 	 LEFT JOIN tb_ykt_temp_wzs_mail FJLB ON FJLB.JXZZJGID = GLY.YX_ID  "+
	...
	public String getWgWzsColumn(String params) {
		...
		String ztsql = "SELECT SUBSTR(XFSJ,0,10) AS LY,'疑似未住宿' as zt,COUNT(*) AS ZS FROM TB_YKT_TEMP_WZS WZSMD "+
" LEFT JOIN TB_JXZZJG ZZJG ON to_char(ZZJG.ID) = WZSMD.YX_ID "+
" WHERE SUBSTR(WZSMD.XFSJ,0,10) BETWEEN  '"+from+"' AND '"+to+"' "+
" GROUP BY SUBSTR(XFSJ,0,10) ORDER BY LY ";// 未住宿
		String ztsql2 = "SELECT SUBSTR(SKSJ,0,10) AS LY,'晚寝晚归' as zt,COUNT(*) AS ZS FROM TB_YKT_TEMP_WG WGMD "+
" LEFT JOIN TB_JXZZJG ZZJG on to_char(ZZJG.ID) = WGMD.YX_ID "+
" WHERE SUBSTR(WGMD.SKSJ,0,10) BETWEEN '"+from+"' AND '"+to+"' "+
" GROUP BY SUBSTR(SKSJ,0,10) ORDER BY LY";// 晚归
		List<Map> ztList = baseDao.querySqlList(ztsql);
		List<Map> ztList2 = baseDao.querySqlList(ztsql2);
		for (Map obj : ztList) {
		...
		result.put("wzs", ztList);
		result.put("wg", ztList2);
