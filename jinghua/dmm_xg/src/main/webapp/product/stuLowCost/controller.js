...
var app = angular.module('app', [ 'ngRoute', 'system' ]).controller(
    ...
    function($scope, service, service_gdxf, advancedService,$timeout,dialog) {
				...
				$scope.data = {
					...
					detailUrl : "pmsStuLowCost/getStuDetail",
					exportUrl : 'pmsStuLowCost/down',
					stu_detail : { 
//						page : {
//							curpage  : 1,
//							pagesize : 10,
...
//						mask   : false
				...
				$scope.headers = ['学号','入学年级','姓名','性别','地区','院系','专业','班级','低消费标准','月总消费金额'];
				...
				$scope.headers2 = ['学号','入学年级','姓名','性别','地区','院系','专业','班级','低消费标准','学期总消费金额'];
...
//				};
				...
				var mealDetail = function(param, obj, data){
					var detail = $scope.data.stu_detail;
					detail.title  = "低消费（"+param.name+"）明细";
					detail.values = {};
					detail.values['ismeal']=1;
					detail.headers= ['学号','入学年级','姓名','性别','地区','院系','专业','班级','低消费标准',param.name+'消费金额'];;
				...
				var gradeDetail = function(param, obj, data){
					var detail = $scope.data.stu_detail;
					detail.title  = "低消费（"+param.name+"）学生名单";
					detail.values = {};
				...
				var sexDetail = function(param, obj, data){
					var detail = $scope.data.stu_detail;
					detail.title  = "低消费（性别："+param.name+"）学生名单";
					detail.values = {};
					detail.headers= $scope.headers2;
					detail.fields = $scope.fields2;
					param.data = obj;
...
//						   distribute.termLoanCfg = data.termLoanCfg;
						var myChart =echarts.init(document.getElementById('Xg_stuLowCost_loan'), 'macarons');
...
//						   distribute.termSubsidyCfg = data.termSubsidyCfg;
						var myChart =echarts.init(document.getElementById('Xg_stuLowCost_subsidy'), 'macarons');
...
//						   distribute.termJmCfg = data.termJmCfg;
						var myChart =echarts.init(document.getElementById('Xg_stuLowCost_jm'), 'macarons');
...
//					detail.page.curpage = 1;
				...
				$scope.exportExcel=function(i){
					...
					var termCode = $scope.data.monthTerm;
					var month =$scope.data.month;
					var mc=map.name+":低消费人数"+map.count+"人，占比"+map.scale+"%, "+schoolYear+"学年"+termCode+"学期 第"+month+"月。";
				...
				$scope.exportAll=function(){
					...
					window.open("stuLowCost/excelAll");
				};
				/**
				 * 单击全部发送按钮
				 */
...
//				};
				$scope.weekLvClick = function(type,id,name){
					...
					if(type=='scale'){
						...
						detail.title = name+"低消费人数占比变化趋势";
					}else{
						$scope.scaleshow = false;
						$scope.changeshow = true;
						detail.title = name+"低消费人数变化趋势";
					}
					getNearLv(id);
				}
