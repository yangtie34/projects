...
public class StuSchLifeDaoImpl implements StuSchLifeDao {
	...
	public List yktxf(String id, String startTime, String endTime) {
		...
		String sql = " SELECT  TCCP.NAME_ field,SUM(TCCP.SURPLUS_MONEY) value ,1 bz "
				+ " FROM (SELECT TCCD.NAME_,TCP.SURPLUS_MONEY "
				+ " FROM T_CARD_PAY TCP "
	...
	public List ieAllTime(String id, String startTime, String endTime) {
		...
		sql="SELECT MIN(TNV.TIME_NUM) MIN_, MAX(TNV.TIME_NUM) MAX_ ,SUM(TNV.TIME_NUM) SUM_ FROM TL_NET_VIEW TNV  GROUP BY TNV.ACCOUNT_";
