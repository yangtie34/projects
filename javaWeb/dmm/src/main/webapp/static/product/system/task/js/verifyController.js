...
app.controller("verifyController", [ "$scope","dialog",'mask','$timeout','http','timeAlert',function(scope,dialog,mask,timeout,http,timeAlert) {
	...
	//分页指令的测试数据
	  ...
	  scope.getVerifyByName=function(){
		  mask.showLoading();
		  htt.verifyByName.params=[];
		  htt.verifyByName.params.push(scope.page.currentPage || 1);
		  htt.verifyByName.params.push(scope.page.numPerPage || 10);
		  htt.verifyByName.params.push(scope.verify.name_);
		  http.callService(htt.verifyByName).success(function(data){
			  vm.items=data.resultListObject;
			  scope.page.totalRows=data.totalRows;
			  scope.page.totalPages=data.totalPages;
			  mask.hideLoading();
