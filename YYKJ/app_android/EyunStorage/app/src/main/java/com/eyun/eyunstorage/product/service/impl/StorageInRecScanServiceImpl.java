...
import com.eyun.eyunstorage.product.dao.impl.ProductStockChangeDaoImpl;
...
import com.eyun.eyunstorage.product.dao.impl.StorageInRecScanDaoImpl;
import com.eyun.eyunstorage.product.dao.impl.StorageLocationChangeDaoImpl;
...
import com.eyun.eyunstorage.product.service.StorageInRecScanService;
...
import static com.eyun.eyunstorage.R.raw.cosign_number_full;
...
public class StorageInRecScanServiceImpl implements StorageInRecScanService {
    ...
    @Override
    public ResultMsg getScanbyRecNumberAndProBarCodeForScanInRec(String RecNumber, String ProBarCode, int ScanType,long ComCusID,int number) {
        Product product = ProductServiceImpl.getInstance().LoadData(ProBarCode);
        if (product==null) {
            ResultMsg resultMsg = new ResultMsg();
            resultMsg.setMsg("商品信息不存在");
        ...
        if(ComCusID!=product.getVestingComCusID()){
            ResultMsg resultMsg = new ResultMsg();
    ...
    public ResultMsg getFjScanProBarCode(String ProBarCode) {
        ...
        if (product==null) {
            ...
            mo.setProName(product.getProName());
            mo.setCreateTime(DateUtils.getCurrentTime());
            mo.setComCusID(product.getVestingComCusID());
            mo.setRecNumber(recNumber);
            StorageInRecScanDaoImpl.getInstance().addScanInfo(mo);
            ResultMsg resultMsg = new ResultMsg();
            resultMsg.setMsg("扫描成功");
