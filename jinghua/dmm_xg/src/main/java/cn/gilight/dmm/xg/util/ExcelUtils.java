...
//			cell6.setCellStyle(style1);
			...
			String[] titles = {"学年","学号","姓名", "性别", "专业", "班级","欠费类型","欠费金额"};
			...
			if(userList != null){
				for(int j = 0; j < userList.size(); j++){
					...
					cell16.setCellValue(userList.get(j).getCL6());
					HSSFCell cell17 = row.createCell(6);
					cell17.setCellValue(userList.get(j).getCL7());
					HSSFCell cell18 = row.createCell(7);
					cell18.setCellValue(userList.get(j).getCL8());
				}
			}
			//输出到硬盘
...
//			cell6.setCellStyle(style1);
	 ...
	 * 导出用户的所有数据到指定的路径
	 */
	public static void exportCostExcel(String title,List<ExcelParam> userList,String filename) {
		try {
...
//			cell6.setCellStyle(style1);
	 ...
	 * 导出用户的所有数据到指定的路径
	 */
	public static void exportCostExcel(String title,List<ExcelParam> userList,ServletOutputStream outputStream) {
		try {
