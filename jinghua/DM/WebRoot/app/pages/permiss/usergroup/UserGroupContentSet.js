NS.define('Pages.permiss.usergroup.UserGroupContentSet',{
    showGroupContentSetPage : function(funId,group){
        var me = this,
            hash = this.nodeHash,
            fun = hash[funId],//功能
...
//        //布局组件
        var layout  = this.roleSetLayout = this.getContentSetLayout(menu,fun,group,contentList,ownerContentCom,hash);
    },
    /**
     * 获取角色设置布局页面
     * @returns {LayoutComponent}
     */
    getContentSetLayout : function(menu,fun,group,contentList,ownerContentCom,hash){
        //布局组件
        var me = this,
            layout  = this.contentSetLayout = new Pages.permiss.base.LayoutComponent({
                layoutTpl : this.contentSetTpl,
                renderNumber : 1,
                data : {jsmc : group.groupname,cdmc : menu.mc,gnmc : fun.mc,contentList :contentList}
            });
        //组件注册进入布局容器中
