...
import cn.gilight.dmm.teaching.entity.TStuGraduateResultDept;
...
@Service("graduateResultDeptJob")
public class GraduateResultDeptJob {
	@Resource
	...
	public JobResultBean doNow(){
		...
		List<TStuGraduateResultDept> all = new ArrayList<TStuGraduateResultDept>();
		for(String year : yearList){
		     List<TStuGraduateResultDept> list = getList(year); 
		     all.addAll(list);
		}
	...
	private JobResultBean doResult(List<TStuGraduateResultDept> list,String jobName){
	...
	private List<TStuGraduateResultDept> getList(String schoolYear){
		List<TStuGraduateResultDept> result = new ArrayList<TStuGraduateResultDept>();
		List<Map<String,Object>> list = bysDegreeService.getListByDeptList(businessDao.queryYxList(PmsUtils.getPmsAll()), schoolYear, new ArrayList<AdvancedParam>()),
		                         list1 = bysDegreeService.getListByDeptList(businessDao.queryZyListStu(PmsUtils.getPmsAll()), schoolYear, new ArrayList<AdvancedParam>());;
	    list.addAll(list1);
	    if(list != null && !list.isEmpty()){
	    	baseDao.delete("Delete from t_stu_graduate_result_dept t where t.school_year = '"+schoolYear+"'");
	    }
		for(Map<String,Object> map :list){
			TStuGraduateResultDept	scoreMap = new TStuGraduateResultDept();
			scoreMap.setDept_id(MapUtils.getString(map, "id"));
