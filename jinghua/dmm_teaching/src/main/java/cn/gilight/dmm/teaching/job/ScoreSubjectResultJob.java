...
@Service("scoreSubjectResultJob")
public class ScoreSubjectResultJob {
		@Resource
		...
		@Transactional
		public JobResultBean doNowSubject(){
			String jobName = " t_stu_score_result_subject各学科当前学期成绩结果";
			List<TStuScoreResultSubject> list1 = new ArrayList<TStuScoreResultSubject>(); 
			...
			for(Map<String, Object> temp1 : temp){
				...
				List<TStuScoreResultSubject> list = getList(schoolYear, termCode, year);
			list1.addAll(list);
			}
		...
		@Transactional
		public JobResultBean doLsSubject(){
			String jobName = " t_stu_score_result_subject各学科历史学期成绩结果";
			List<TStuScoreResultSubject> list1 = new ArrayList<TStuScoreResultSubject>(); 
			List<Map<String, Object>> temp = getYearAndTerm();
			if(temp.size()>1){
		...
		@Transactional
		public JobResultBean doNowYearSubject(){
			String jobName = " t_stu_score_result_subject各学科当前学年成绩结果";
			List<TStuScoreResultSubject> list1 = new ArrayList<TStuScoreResultSubject>(); 
		...
		@Transactional
		public JobResultBean doLsYearSubject(){
			String jobName = " t_stu_score_result_subject各学科历史学年成绩结果";
			List<TStuScoreResultSubject> list1 = new ArrayList<TStuScoreResultSubject>(); 
			List<Map<String, Object>> temp = getYear();
			if(temp.size()>1){
				temp = temp.subList(1, temp.size());
		...
		private JobResultBean doResult(List<TStuScoreResultSubject> list,String jobName){
		...
		private List<TStuScoreResultSubject> getList(String schoolYear,String termCode,int year){
			List<String> deptList1 = PmsUtils.getPmsAll();
			List<Map<String, Object>> list = businessService.queryBzdmSubjectDegree();
			for(Map<String,Object> map : list){
				List<AdvancedParam> advancedParam = new ArrayList<AdvancedParam>();
				if(MapUtils.getString(map, "id") != null && !"".equals(MapUtils.getString(map, "id"))){
			    	 AdvancedParam advancedSubject = new AdvancedParam(AdvancedUtil.Group_Stu, AdvancedUtil.Stu_SUBJECT, MapUtils.getString(map, "id"));
			         AdvancedUtil.add(advancedParam, advancedSubject);
			     }
				...
				if(edu != null && !"".equals(edu)){
			         ...
			         AdvancedUtil.add(advancedParam, advancedEdu);
			     }
				map.put("stuSql", businessDao.getStuSql(year, deptList1, advancedParam));
			}
			    List<TStuScoreResultSubject> list1 = getGridListParamAndValues(list, schoolYear, termCode, new ArrayList<AdvancedParam>());
			    return list1;
		}
		/** 计算 院系/专业/班级/学科 绩点列表数据 */
		@SuppressWarnings("unchecked")
		private List<TStuScoreResultSubject> getGridListParamAndValues(List<Map<String, Object>> list, String schoolYear, String termCode,
				 List<AdvancedParam> businessAdvancedList) {
			List<TStuScoreResultSubject> result = new ArrayList<TStuScoreResultSubject>();
			String stuSqlKey = "stuSql", stuSql = "", scoreSql = "", 
					betterSql = "", failSql = "", rebuildSql = "",gpaSql ="",smartSql ="";
			...
			for(Map<String, Object> map : list){
				...
				scoreList  = baseDao.queryForListDouble("select a.score from (select case when sum(t.credit)= 0 then 0 "
						+ " else sum(t.score * t.credit)/sum(t.credit) end as score,t.stu_id from ("+sql+") t group by t.stu_id) a"
				...
				TStuScoreResultSubject scoreMap = getScoreValueMapByTarget(gpaList, scoreSql, betterSql, failSql, rebuildSql,scoreList,gpaSql,smartSql);
				scoreMap.setSchool_year(schoolYear);
				scoreMap.setTerm_code(termCode);
				scoreMap.setSubject_degree_id(MapUtils.getString(map, "id"));
				String termSql = termCode == null?" is null":" = '"+termCode+"'";
				baseDao.delete("delete from T_STU_SCORE_RESULT_SUBJECT t where t.subject_degree_id ='"+MapUtils.getString(map, "id")+"' and t.school_year ='"+schoolYear+"'"
						+ " and t.term_code "+termSql+"");
				result.add(scoreMap);
			}
			return result;
		}
		private TStuScoreResultSubject getScoreValueMapByTarget(List<Double> valueList, String scoreSql, 
				String betterSql, String failSql, String rebuildSql, List<Double> scoreList,String gpaSql,String smartSql) {
			TStuScoreResultSubject	scoreMap = new TStuScoreResultSubject();
