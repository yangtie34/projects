    ...
    public void okScan(final String code) {
        ...
        if(StringUtils.hasLength(recNumber)){//扫描商品处理方法
            callBack=new CallBack() {
                @Override
                public Object run() {
                    ResultMsg resultMsg=StorageInRecScanServiceImpl.getInstance().getScanbyRecNumberAndProBarCodeForScanInRec(recNumber,code, AppPublic.ScanType.FJ,ComCusID,someModeNumber);
                    return resultMsg;
                }
            };
            ...
            scope.forThread(new CallBack<ResultMsg>() {
                ...
                public void hasChange(ResultMsg resultMsg) {
                    ...
                    if(resultMsg.isTure()){
                        ...
                        ComCusID=storageInRecScan.getComCusID();
                        initThisScan(storageInRecScan);
                        initListView();
                    }else{
