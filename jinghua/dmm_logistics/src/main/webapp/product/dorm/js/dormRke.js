app.controller("dormRkeController", [ "$scope","dialog",'mask','$timeout','http','timeAlert','exportPage',function(scope,dialog,mask,timeout,http,timeAlert,exportPage) {
	mask.showLoading();
	  scope.page = {
			  totalPages : 0,
			  totalRows : 0,
	 ...
	 var vm = scope.vm = {};
	  vm.items = [];
	  var htt=[];
	  var httservice="dormRkeUsedService?"; //宿舍门禁使用情况
	  var methods=[	
	        	'getDormRkeUsed',			//0住宿学生情况
	        	'getDormRkeUsedBySex',		//1分性别使用情况
	        	'getDormRkeUsedByEdu',		//2分学历使用情况
	        	'getDormRkeUsedByMz',		//3分民族使用情况
	        	'getDormRkeUsedByDept',	//4分学院使用情况
	        	'getDormRkeUsedPage',		//5活跃人群
	        	'getNoDormRkeUsed',		//6不活跃人群
	               ];
...
var title,titles,titlesCode;
var query=function(pg){
	var i=5;
	 scope.page1=angular.copy(pg);
	 if(pg.exportExcel){
...
var getvmData=function(i){
	http.callService(htt[i]).success(function(data){
		 if(i<3){
			 ...
			 data.titles=titles;
			 data.titlesCode=titlesCode;
			 data.func=query;
	  		  scope.pagexq=angular.copy(data);
		 }else if(i==6){
...
scope.exportExcel=function(){
	 mask.showLoading();
var invoke=angular.copy(htt[6]);
invoke.params[0]=1;
invoke.params[1]=scope.page.totalRows;
var ex={
	 invoke:invoke,
	 title:'一卡通非活跃学生名单',
	 titles:'学号,姓名,性别,所属学院,专业,班级,住宿楼宇,出入次数'.split(','),
	 titleCodes:'STU_ID,STU_NAME,SEX_NAME,DEPT_NAME,MAJOR_NAME,CLASS_NAME,LY_NAME,RKE_COUNT'.split(',')
}
mask.hideLoading(); 
...
var getparams=function(i){
		 ...
		 if(i<5){
			 ...
			 params=[scope.page1.currentPage || 1,
			         scope.page1.numPerPage || 10,0,
			         startDate,endDate,deptTeach,scope.type,scope.type_code];
		 }else if(i==6){
