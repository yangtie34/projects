 ...
 * 奖助-基础模板页面[zhangyc]
 */
NS.define('Business.xg.jczd.JzModel', {
	extend : 'Template.Page',
	requires:['Business.component.PageHeader','Business.component.PageBottom','Business.component.PageHeaderXnXq'],
	//预加载tpl
	tplRequires : [
	 ...
	 cssRequires : ['app/business/xg/jczd/template/style/zxj.css',
                    ...
                    'app/business/xg/template/style/wbh-common.css'
	                ],
	/**
	 * 请求后台服务配置
	 */
    ...
    initTplRequires:function(){
        this.tplRequires = NS.Array.merge(this.addTplRequires,this.tplRequires);
	...
	initPage : function(){
		...
		this.page.on('afterrender',function(){
				if(this.cdmc!=''){
                    if(NS.isDefined(this.topCom)){
                        this.topCom.render(this.zxj_top);//顶部
                    }
				}
				if(this.select!=''){
                    if(NS.isDefined(this.selectCom)){
					    this.selectCom.render(this.zxj_select);//切换选择
                    }
                    if(NS.isDefined(this.selectlbCom)){
				...
				if(this.ms!=''){
                    if(NS.isDefined(this.bottomCom)){
