...
import cn.gilight.dmm.teaching.entity.TStuScoreResultTeacher;
...
@Service("scoreTeaResultJob")
public class ScoreTeaResultJob {
	@Resource
	...
	@Transactional
	public JobResultBean doNowTea(){
		String jobName = " t_stu_score_result_Teacher各教师当前学期成绩结果";
		List<TStuScoreResultTeacher> list1 = new ArrayList<TStuScoreResultTeacher>(); 
		...
		for(Map<String, Object> temp1 : temp){
			...
			List<TStuScoreResultTeacher> list = getGridListParamAndValues(year,schoolYear, termCode, new ArrayList<AdvancedParam>());
		list1.addAll(list);
		}
	...
	@Transactional
	public JobResultBean doLsTea(){
		String jobName = " t_stu_score_result_Teacher各教师历史学期成绩结果";
		List<TStuScoreResultTeacher> list1 = new ArrayList<TStuScoreResultTeacher>(); 
		List<Map<String, Object>> temp = getYearAndTerm();
		if(temp.size()>1){
	...
	@Transactional
	public JobResultBean doNowYearTea(){
		String jobName = " t_stu_score_result_Teacher各教师当前学年成绩结果";
		List<TStuScoreResultTeacher> list1 = new ArrayList<TStuScoreResultTeacher>(); 
	...
	@Transactional
	public JobResultBean doLsYearTea(){
		String jobName = " t_stu_score_result_Teacher各教师历史学年成绩结果";
		List<TStuScoreResultTeacher> list1 = new ArrayList<TStuScoreResultTeacher>(); 
		List<Map<String, Object>> temp = getYear();
		if(temp.size()>1){
			temp = temp.subList(1, temp.size());
	...
	private JobResultBean doResult(List<TStuScoreResultTeacher> list,String jobName){
	...
	private List<TStuScoreResultTeacher> getGridListParamAndValues(int year, String schoolYear, String termCode,
			 List<AdvancedParam> businessAdvancedList) {
		 String baseStuSql = businessDao.getStuSql(year, PmsUtils.getPmsAll(), new ArrayList<AdvancedParam>()),
			       scoreSql = "", betterSql = "", failSql = "", rebuildSql = "",
			       stuSql = "",gpaSql = "",smartSql = "";
		 List<Map<String, Object>> list = scoreDao.queryTeachList(baseStuSql, schoolYear, termCode);
		 List<TStuScoreResultTeacher> result = new ArrayList<TStuScoreResultTeacher>();
		...
		for(Map<String, Object> map : list){
			...
			TStuScoreResultTeacher scoreMap = getScoreValueMapByTarget(gpaList, scoreSql, betterSql, failSql, rebuildSql,scoreList,gpaSql,smartSql);
			scoreMap.setSchool_year(schoolYear);
			scoreMap.setTerm_code(termCode);
			scoreMap.setTea_id(MapUtils.getString(map, "id"));
			String termSql = termCode == null?" is null":" = '"+termCode+"'";
			baseDao.delete("delete from T_STU_SCORE_RESULT_TEACHER t where t.tea_id ='"+MapUtils.getString(map, "id")+"' and t.school_year ='"+schoolYear+"'"
					+ " and t.term_code "+termSql+"");
			result.add(scoreMap);
		}
		return result;
	}
	private TStuScoreResultTeacher getScoreValueMapByTarget(List<Double> valueList, String scoreSql, 
			String betterSql, String failSql, String rebuildSql, List<Double> scoreList,String gpaSql,String smartSql) {
		TStuScoreResultTeacher	scoreMap = new TStuScoreResultTeacher();
