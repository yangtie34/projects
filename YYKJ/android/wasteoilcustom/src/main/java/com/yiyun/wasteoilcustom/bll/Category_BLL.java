...
public class Category_BLL {
     ...
     * @return
     */
    public static ArrayList<TreeEntity> GetProductCategory(long comId)
    {
            ...
            if(!jsonStr.trim().equals(""))
            {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    if (jsonObject.has("Name")) {
                        value.setName(jsonObject.get("Name").toString().replace("\uE5E5","    "));
                    } else {
                        continue;

                    }
                    if (jsonObject.has("Code")) {
                        value.setId(jsonObject.get("Code").toString());
                    } else {
                        continue;
                    }
                    list.add(value);
