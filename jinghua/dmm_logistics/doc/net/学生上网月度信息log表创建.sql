...
select to_date('2013-09','yyyy-mm') year_month,
 ...
 where  substr(cp.on_time, 0, 7)='2013-09' and s.no_ is not null 
 group by substr(cp.on_time, 0, 7),
       s.no_,
       s.name_,
       s.sex_code,
       tc.name_ ,
...
drop table tl_net_stu_month;
create table tl_net_stu_month ( 
  YEAR_MONTH      date, 		--年月
  USE_TIME	  	  VARCHAR2(100),--在线时长(该月总在线时长)
  USE_FLOW	  	  VARCHAR2(100),--使用流量
  USE_MONEY	  	  VARCHAR2(100),--消费金额
  IN_COUNTS		  VARCHAR2(100),--登陆次数
  STU_ID          VARCHAR2(100),
  STU_NAME        VARCHAR2(100),--学生
