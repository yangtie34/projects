...
directives.directive('csWindow',['$timeout','$compile',function($timeout,$compile){
    ...
    return {
        ...
        link : function(scope, element, attrs){
            ...
            var mask = $('<div class="cs-window-background" id="maskId"></div>');
           // var mask = $('<div class="cs-window-background" id="'+scope.$id+'"></div>');
            mask.hide();
            if($("#maskID").length==0){
            	 body.append(mask);
            }else{
            	mask=$("#maskID");mask.hide();
            }
            ...
            scope.$watch('show',function(newV,oldV){
                if(newV == true){
                    ...
                    element.find(".modal-header button.close").on('click',function(){
                     	scope.show = false;
                     	$timeout(function(){
                			scope.$apply();
                		});
